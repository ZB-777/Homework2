...
為了實際驗證效能，我針對不同項數的多項式進行測試，  
觀察運算時間及執行穩定度（使用 Visual Studio 的時間統計與人工觀察）。
 ...
 **觀察結果：**
- 加法的時間隨項數線性增加，與理論 O(n) 相符。  
- 乘法的時間約呈平方關係，與理論 O(n²) 符合。  
- 程式在 100 項以內運行穩定，沒有記憶體洩漏或越界問題。  
- 當項數增加時，主要延遲來自於 `newTerm()` 的重複搜尋與動態擴充。
